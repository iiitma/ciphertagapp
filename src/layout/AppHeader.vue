      ...
      <ul class="navbar-nav align-items-lg-center ml-lg-auto">
        ...
        <li class="nav-item">
          <router-link class="nav-link nav-link-icon" to="/howtoplay">
            <span class="nav-link-inner--text spartan-medium "
              >How to play</span
            >
          </router-link>
        </li>
        ...
        <li class="nav-item d-none d-lg-block" v-if="user != null">
          <span class="d-none text-white mr-3">{{
            user.split(":")[0] || 0
          }}</span>
        ...
        <li class="nav-item d-sm-block d-lg-none d-md-none" v-if="user != null">
          <a
            @click="logout()"
            class="btn btn-sm btn-primary text-white mt-3 text-capitalize spartan-regular"
            >Logout</a
          >
        </li>
      </ul>
...
export default {
  ...
  methods: {
    ...
    getUser() {
      setInterval(() => {
        this.user = JSON.parse(localStorage.getItem("gamerid"));
      }, 3000);
    },
  },
  mounted() {
