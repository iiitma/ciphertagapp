        ...
        <div id="game-view" class="player">
          <div id="status-line" v-if="winner[0]">
            <div class="btn-group">
              <base-button
                type="twitter"
                size=""
                class="text-capitalize spartan-extrabold"
                >{{ game.points[0] }}</base-button
              >
              <base-button
                type="youtube"
                size=""
                class="text-capitalize spartan-extrabold"
                >{{ game.points[1] }}</base-button
              >
            </div>

            <span class="mx-auto spartan-bold text-white text-capitalize"
              >!!! {{ winner[1] }}</span
            >
          </div>
          <div id="status-line" v-else>
            ...
            <div id="status" class="text-white">
              <span v-if="convertToTurn(user.team) == game.turn">Your</span
              ><span v-else-if="game.turn == 1">Blue's</span>
              <span v-else-if="game.turn == 2">Reds's</span> turn
            </div>
            ...
            <base-button
              v-if="
                game.turn == 1 &&
                (user.role == 'spymaster' ||
                  convertToTurn(user.team) == game.turn)
              "
              @click="endturn(1)"
              type="white"
              size="sm"
              ...
              >End
              <span v-if="convertToTurn(user.team) != 1">blue's</span>
              turn</base-button
            >
            <base-button
              v-if="
              ...
              class="text-capitalize"
              >End
              <span v-if="convertToTurn(user.team) != 2">red's</span>
              turn</base-button
            >
          </div>
          <!--div-- >
          ...
          <div v-else>
            <div v-if="user.role == 'spymaster'" class="board">
              <div
                class="cell spymaster"
                :class="word[1] + ' ' + word[2]"
                v-for="word in game.words"
                :key="word.join('-')"
              >
                <span
                  class="word small spartan-bold text-capitalize"
                  role="button"
                  aria-disabled="true"
                  :aria-label="word[0] + ',' + word[1] + ', revealed'"
                  >{{ word[0] }}</span
                >
              </div>
            </div>
            <div v-else class="board">
              ...
              >
                <span
                  class="word small spartan-medium text-capitalize"
                  role="button"
                  aria-disabled="true"
                  :aria-label="word[0] + ',' + word[1] + ',' + word[2]"
                  >{{ word[0] }}</span
                >
              </div>
            </div>
          ...
          >
            <base-button
              ...
              size=""
              class="text-capitalize"
              >Next Game</base-button
            >
            <base-button
              @click="close()"
              type="outline-white"
              class="text-capitalize spartan-regular"
              >Close Room</base-button
            >
          </div>
          <div class="mt-5">
            <label class="text-white spartan-medium"
            ...
            <div class="row">
              <div class="col-10 mr-0">
                <base-input
                  ...
                  :value="'https://ciphertag.herokuapp.com/join/' + roomid"
...
export default {
  ...
  methods: {
    ...
    flip(word) {
      if (!this.winner[0] && word[2] == "hidden") {
